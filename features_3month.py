...
for i in range(89,len(prev_close_list)):
    result=0
    for j in range(i-90+1,i):
        result=result+(close_list[0][j]-close_list[1][j])/close_list[1][j]
    volatility=result/90
...
for i in range(89,len(flag_list)):
    result=0
    for j in range(i-90+1,i):
        result=result+flag_list[j]
    momentum_list.append(result/90)
print momentum_list
...
for i in range(89,len(percent_change_list)):
    result=0
    for j in range(i-90+1,i):
        result=result+percent_change_list[j]
    volatility_index=result/90
...
for i in range(89,len(flag_list_index)):
    result=0
    for j in range(i-90+1,i):
        result=result+flag_list_index[j]
    momentum_list_index.append(result/90)
print momentum_list_index
print (len(momentum_list_index))

del close_price_list[:89]
del prev_close_list[:89]
del date_list[:89]
del symbol_list[:89]

with open('Features_stock/features_3month_csv.csv', 'a') as csvfileWrite:
