...
def method(interval,Company_file,Index_file,output_dir):
    ...
    for i in range(interval-1,len(flag_list)):
        result=0
        for j in range(i-interval+1,i):
            result=result+flag_list[j]
        momentum_list.append(result/interval)
    print momentum_list
    ...
    for i in range(interval-1,len(percent_change_list)):
        result=0
        for j in range(i-interval+1,i):
            result=result+percent_change_list[j]
        volatility_index=result/interval
    ...
    for i in range(interval-1,len(flag_list_index)):
        result=0
        for j in range(i-interval+1,i):
            result=result+flag_list_index[j]
        momentum_list_index.append(result/interval)
    print momentum_list_index
    print (len(momentum_list_index))

    del symbol_list[:(interval-1)]
    del date_list[:(interval-1)]
    del prev_close_list[:(interval-1)]
    del open_price_list[:(interval-1)]
    del high_price_list[:(interval-1)]
    del low_price_list[:(interval-1)]
    del last_price_list[:(interval-1)]
    del average_price_list[:(interval-1)]
    del total_traded_quantity_list[:(interval-1)]
    del close_price_list[:(interval-1)]
